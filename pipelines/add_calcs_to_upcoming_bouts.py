...
# append upcoming_bouts minus 'cage_size' to a duplicate
# of raw_ufc_data, let the column names
# which are not in upcoming_bouts have null values

# create a table called upcoming_bout_calcs
...
def create_upcoming_bout_calcs(cursor):
    try:
        # Check if the 'upcoming_bout_calcs' table exists and drop it if it does
        cursor.execute("DROP TABLE IF EXISTS upcoming_bout_calcs")

        # Duplicate raw_ufc_data table as raw_ufc_data_duplicate
        cursor.execute("CREATE TABLE upcoming_bout_calcs AS SELECT * FROM raw_ufc_data")

    except sqlite3.Error as e:
        print(f"An error occurred: {e}")

# append data from table 'upcoming_bouts' to table 'upcoming_bout_calcs'
def append_upcoming_bouts(cursor):
    try:
        cursor.execute("INSERT INTO upcoming_bout_calcs (event_title, date, location, fighter_a_name, fighter_b_name, weight_class) SELECT event_title, date, location, fighter_a_name, fighter_b_name, weight_class FROM upcoming_bouts")
...
def clean_weight_class(cursor):
    try:
        # Check if the 'upcoming_bout_calcs' table exists and drop it if it does
        #cursor.execute("DROP TABLE IF EXISTS upcoming_bout_cals")

        # Duplicate raw_ufc_data table as upcoming_bout_calcs
        #cursor.execute("CREATE TABLE upcoming_bout_calcs AS SELECT * FROM raw_ufc_data")
        ...
        for substring in substrings_to_remove:
            cursor.execute(f"UPDATE upcoming_bout_calcs SET weight_class = REPLACE(weight_class, '{substring}', '')")
...
def determine_cage_size(cursor):
    ...
    # Add a column called 'cage_size' to the upcoming_bout_calcs table
    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN cage_size TEXT")
    except sqlite3.Error as e:
        print("Error adding column 'cage_size' to upcoming_bout_calcs:", e)
        return

    # Update the cage_size column for each row
    try:
        cursor.execute("UPDATE upcoming_bout_calcs SET cage_size = 'small' WHERE date BETWEEN '05-30-2020' AND '06-27-2020'")
        cursor.execute("UPDATE upcoming_bout_calcs SET cage_size = 'small' WHERE date > '03-20-2020' AND event_title LIKE '%Fight Night%' AND location LIKE '%Las Vegas%'")
        cursor.execute("UPDATE upcoming_bout_calcs SET cage_size = 'big' WHERE cage_size IS NULL")
    except sqlite3.Error as e:
        print("Error updating 'cage_size' in upcoming_bout_calcs:", e)
        return


def fighter_age_at_bouts(cursor):

    # Ensure the 'fighter_a_bout_age' and 'fighter_b_bout_age' columns exist in 'upcoming_bout_calcs'
    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN fighter_a_bout_age REAL")
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN fighter_b_bout_age REAL")
    except sqlite3.Error:
        # Columns already exist, continue
        pass

    # Retrieve 'date', 'fighter_a_name', and 'fighter_b_name' from 'upcoming_bout_calcs'
    cursor.execute("SELECT date, fighter_a_name, fighter_b_name FROM upcoming_bout_calcs")
    ...
    for fight_date, fighter_a, fighter_b in fights:
        ...
        # Update 'fighter_a_bout_age' and 'fighter_b_bout_age' in 'upcoming_bout_calcs'
        cursor.execute("UPDATE upcoming_bout_calcs SET fighter_a_bout_age = ?, fighter_b_bout_age = ? WHERE date = ? AND fighter_a_name = ? AND fighter_b_name = ?", (age_at_bout_a, age_at_bout_b, fight_date, fighter_a, fighter_b))
...
def age_difference(cursor):
    ...
    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN fighter_a_age_diff REAL")
    except sqlite3.OperationalError:
        pass  # Column already exists, so ignore the error

    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN fighter_b_age_diff REAL")
    except sqlite3.OperationalError:
        pass  # Column already exists, so ignore the error

    # Update and round the values in the columns
    cursor.execute("UPDATE upcoming_bout_calcs SET fighter_a_age_diff = ROUND(fighter_a_bout_age - fighter_b_bout_age, 2)")
    cursor.execute("UPDATE upcoming_bout_calcs SET fighter_b_age_diff = ROUND(fighter_b_bout_age - fighter_a_bout_age, 2)")
...
def duration_seconds(cursor):
    ...
    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN duration_seconds INTEGER")
    except sqlite3.Error:
        # Columns already exist, continue
        pass
    try:
        # Retrieve 'date', 'fighter_a_name', and 'fighter_b_name' from 'upcoming_bout_calcs'
        cursor.execute("SELECT round_text, time_text FROM upcoming_bout_calcs")
        fights = cursor.fetchall()

        for round_text, time_text in fights:
            
            if round_text is None or time_text is None:
                continue
            ...
            cursor.execute("UPDATE upcoming_bout_calcs SET duration_seconds=? WHERE round_text=? AND time_text=?", (duration_seconds, round_text, time_text))
...
def distance_bool(cursor):
    ...
    # Ensure the 'distance_bool' column exists in 'upcoming_bout_calcs'
    try:
        cursor.execute("ALTER TABLE upcoming_bout_calcs ADD COLUMN distance_bool INTEGER")
    except sqlite3.Error:
        pass  # Column already exists, continue

    # Retrieve 'round_text' and 'time_text' from 'upcoming_bout_calcs'
    cursor.execute("SELECT round_text, time_text FROM upcoming_bout_calcs")
    fights = cursor.fetchall()
    ...
    for round_text, time_text in fights:
        ...
        cursor.execute("UPDATE upcoming_bout_calcs SET distance_bool=? WHERE round_text=? AND time_text=?", (distance_value, round_text, time_text))
...
def update_database(cursor, date, fighter_a, fighter_b, a_eff_diff, a_ctrl_diff):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE upcoming_bout_calcs
            SET fighter_a_eff_diff = ?,
                fighter_a_control_rate_diff = ?
            WHERE date = ? AND fighter_a_name = ? AND fighter_b_name = ?''', 
            (a_eff_diff, a_ctrl_diff, date_str, fighter_a, fighter_b))
...
def calculate_efficiency_and_control_rate(cursor):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM upcoming_bout_calcs')
        ...
        for fight in fights:
            ...
            update_database(cursor, date, fighter_a, fighter_b, a_eff_diff, a_ctrl_diff)
...
def get_upcoming_bouts(cursor):
    # Retrieve only upcoming bouts and the columns needed for the model
    current_date = datetime.now().strftime('%Y-%m-%d')

    # create a table called upcoming_bouts_for_model and insert the data there
    cursor.execute("DROP TABLE IF EXISTS upcoming_bouts_for_model")
    cursor.execute("""
        CREATE TABLE upcoming_bouts_for_model AS 
        SELECT 
            date, 
            fighter_a_name, 
            fighter_b_name, 
        ...
        FROM 
            upcoming_bout_calcs 
        WHERE 
            STRFTIME('%Y-%m-%d', SUBSTR(date, 7, 4) || '-' || SUBSTR(date, 1, 2) || '-' || SUBSTR(date, 4, 2)) > ?
    """, (current_date,))
...
def main():
    ...
    try:
        ...
        create_upcoming_bout_calcs(cursor)
        append_upcoming_bouts(cursor)
        ...
        # New function calls
        add_column_if_not_exists(cursor, 'upcoming_bout_calcs', 'fighter_a_eff_diff', 'REAL')
        #add_column_if_not_exists(cursor, 'upcoming_bout_calcs', 'fighter_b_eff_diff', 'REAL')
        add_column_if_not_exists(cursor, 'upcoming_bout_calcs', 'fighter_a_control_rate_diff', 'REAL')
        #add_column_if_not_exists(cursor, 'upcoming_bout_calcs', 'fighter_b_control_rate_diff', 'REAL')

        calculate_efficiency_and_control_rate(cursor)
        get_upcoming_bouts(cursor)
